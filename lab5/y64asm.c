...
void saveData(byte_t *code, long value, int bytes){
    for(int i = 0; i < bytes ; i ++){
        code[i] = ((value >> (i * 8)) & 0xff);
    }
}
...
int add_symbol(char *name)
{
    ...
    newSym->name = (char*)malloc(sizeof(strlen((name)+1)));
...
parse_t parse_instr(char **ptr, instr_t **inst)
{
    ...
    instr_t * findInst = find_instr(curP);
    if(findInst == NULL)     // 没有这条instr
        return PARSE_ERR;
    
    curP += findInst->len;
    if(!IS_END(curP) && !IS_BLANK(curP) && !IS_COMMENT(curP))   //  ????
        return PARSE_ERR;

    /* set 'ptr' and 'inst' */
    *ptr = curP;  //  把asm里的 instruction copy到ptr对应的内存中
    *inst = findInst;
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
   ...
   char * curP = *ptr;
    ...
    /* find register */
    reg_t * findReg = find_register(curP);
    if(findReg == NULL) return PARSE_ERR;
     regid_t findRegId =  findReg->id;

    /* set 'ptr' and 'regid' */
         curP += findReg->namelen;
         *regid = findRegId;
         *ptr = curP;
         return PARSE_REG;
}
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    if(IS_END(curP))  
        return PARSE_ERR;
    if(!IS_LETTER(curP)) return PARSE_ERR;
    /* allocate name and copy to it */
    int lenOfSym = 0;
    while (!IS_BLANK(curP) && !IS_END(curP) && *curP != ',')
    {
        curP++;
        lenOfSym++;
    }
    
    //symbol_t *findSymbol = find_symbol(curP);
    /* set 'ptr' and 'name' */

        // do we need to new another char*?
        *name = (char*)malloc(lenOfSym + 1);
        strncpy(*name, *ptr,lenOfSym);
        *(*name + lenOfSym) = '\0';
        *ptr = curP;
        return PARSE_SYMBOL;
}
...
parse_t parse_digit(char **ptr, long *value)
{
    ...
    if(IS_END(curP))  
        return PARSE_ERR;
    if(!IS_DIGIT(curP)) return PARSE_ERR;
    /* calculate the digit, (NOTE: see strtoll()) */
    long num = 0;
    num = (long)strtoul(curP, &curP, 0);
    /* set 'ptr' and 'value' */
    *ptr = curP;
    *value = num;
    return PARSE_DIGIT;
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
    ...
    {
        (*ptr)++;
        if(PARSE_DIGIT == parse_digit(ptr, value))
        {
            return PARSE_DIGIT;
        }
        (*ptr)--;
    ...
    if(IS_LETTER(*ptr))
    {
        if(PARSE_SYMBOL == parse_symbol(ptr, name))
        {
            return PARSE_SYMBOL;
        }
    }
...
parse_t parse_label(char **ptr, char **name)
{
    ...
    char* labelName;
    SKIP_BLANK(temp);
    if(!IS_LETTER(temp)) return PARSE_ERR;
    while(!IS_END(temp) && !IS_BLANK(temp)){
        if(*temp == ':'){
            labelName = (char*)malloc(len+1);
            memset(labelName, '\0', len + 1);
            memcpy(labelName, *ptr, len);
            *name = malloc(len + 1);
            *name = labelName;
            *ptr = temp + 1;
            return PARSE_LABEL;
...
*           call SUM  #invoke SUM function */
    ...
    if(parse_instr(&ptr, &inst) == PARSE_INSTR){
        ...
        case I_IRMOVQ:
            ...
            if(type == PARSE_DIGIT)
                saveData(&(line->y64bin.codes[2]),value,8);
        ...
        case I_CALL:
            ...
            if(type == PARSE_DIGIT)
                saveData(&(line->y64bin.codes[1]),value,8);
            else if (type == PARSE_SYMBOL)
                add_reloc(name, &line->y64bin);
            break;
        case I_PUSHQ:
        ...
        case I_DIRECTIVE:
        {
            ...
            case D_DATA:
                ...
                if(type == PARSE_DIGIT)
                    saveData(&(line->y64bin.codes[0]),value,inst->bytes);
            ...
            case D_ALIGN:
                ...
                if(vmaddr % (int)(value) != 0){  // 没有对齐
                    vmaddr += (value -  vmaddr % (int)value);
                }
                line->y64bin.addr = vmaddr; 
                break;
            default:
...
{
    reloc_t *rtmp = reltab;
    symbol_t * symTem = NULL;
    while (rtmp->next) {
        /* find symbol */
        symTem = find_symbol(rtmp->name);
        /* relocate y64bin according itype */
        if(symTem == NULL){
           err_print("Unknown symbol:'%s'", rtmp->name);
            return -1;
        }
        int len = rtmp->y64bin->bytes;
        saveData(&(rtmp->y64bin->codes[len>8?(len-8):0]),symTem->addr,(len > 4) ? 8 : len);
        /* next */
        rtmp = rtmp->next;
    }
    return 0;
