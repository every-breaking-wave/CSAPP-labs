...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    if((op == A_ADD && !(argA^argB) && (argB^val))   // check if overflow
...
stat_t nexti(y64sim_t *sim)
{
    ...
    regid_t regA_id = 15, regB_id = 15;
    long_t regA_val = 0, regB_val = 0;
    long_t imm = 0;
    long_t long_val = 0;
  //  printf("nextpc is %d", next_pc);
    ...
    icode = GET_ICODE(codefun);   // instruction的id
    ifun = GET_FUN(codefun);      // 第二位的id
    next_pc++;  

    if(icode >= 2 && icode <= 6 || icode >= 10 && icode <= 11){
	get_byte_val(sim->m, next_pc, &codefun);
        regA_id = GET_REGA(codefun);
        regB_id = GET_REGB(codefun);
        regA_val = get_reg_val(sim->r,regA_id);
        regB_val = get_reg_val(sim->r,regB_id);
        next_pc++;
    }
   ...
   if(next_pc >= MEM_SIZE || next_pc <0){
    ...
    switch (icode) {
      ...
      if(cond_doit(sim->cc, ifun))
       ...
       // set_reg_val(sim->r, regB_id, regB_val);
        sim->pc = next_pc;
        break;

      case I_IRMOVQ: /* 3:0 F:regB imm */
       if((imm >= MEM_SIZE || imm < 0 ) && regB_id == 4) {
	    err_print("PC = 0x%lx, Invalid stack address 0x%lx",sim->pc, imm);
            return STAT_ADR;
	   }
        set_reg_val(sim->r, regB_id, imm);
        sim->pc = next_pc;
        break;

      case I_RMMOVQ: /* 4:0 regA:regB imm */
	    set_long_val(sim->m, get_reg_val(sim->r,regB_id)+imm, regA_val);
        sim->pc = next_pc;
        break;

      case I_MRMOVQ: /* 5:0 regB:regA imm */
         if(!get_long_val(sim->m, get_reg_val(sim->r, GET_REGB(codefun)) + imm, &long_val)){
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, get_reg_val(sim->r, GET_REGB(codefun)) + imm);
            return STAT_ADR;
         }
        set_reg_val(sim->r, regA_id, long_val);
        sim->pc = next_pc;
        break;

      case I_ALU: /* 6:x regA:regB */
      long_val = compute_alu(ifun, regA_val,regB_val);
      set_reg_val(sim->r,regB_id,long_val);
      sim->cc = compute_cc(ifun, regA_val, regB_val ,long_val);
      sim->pc = next_pc;
      break;
      ...
      case I_CALL: /* 8:x imm */
        set_reg_val(sim->r,4,get_reg_val(sim->r,4) - 8);
        if(!set_long_val(sim->m, get_reg_val(sim->r, 4), next_pc)){
      ...
      case I_RET: /* 9:0 */
         get_long_val(sim->m, get_reg_val(sim->r,4),&long_val);
         set_reg_val(sim->r, 4, get_reg_val(sim->r,4)+ 8);
         sim->pc = long_val;
         break;

      case I_PUSHQ: /* A:0 regA:F */
         long_val = regA_val;  // set value from reg to mem
         set_reg_val(sim->r,4,get_reg_val(sim->r,4) - 8);
         if(!set_long_val(sim->m, get_reg_val(sim->r, 4), long_val)){
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, get_reg_val(sim->r, REG_RSP));
            return STAT_ADR;
        }
        sim->pc = next_pc;
      ...
      case I_POPQ: /* B:0 regA:F */   // special case : pop %rsp
        get_long_val(sim->m, get_reg_val(sim->r,4),&long_val);
        set_reg_val(sim->r,4, get_reg_val(sim->r, 4) + 8);
        set_reg_val(sim->r, regA_id, long_val);
